...
/*
    ...
    function get_list_of_accounts($sess){
        $data = array(
        ...
        $return_object = json_decode($resp, true);
    ...
    */
    ...
    function create_lte_user_test($sess, $userName, $realm, $password, $class_id, $name, $id, $cell,
                                  $day, $email, $adrType, $adrStr, $suburb, $city, $postalCode, $ricaAdr, $dev) {

        $data = array(
            'function' => 'addLTEAccount',
            'strSessionID'   => $sess,
            'UserName' => $userName,
            'Realm' => $realm,
            'Password' => $password,
        ...
        $resp = curl_exec($handle);
        curl_close($handle);
        echo "<pre>";var_dump($resp, $data);die();
        return json_decode($resp, true);
    }
    ...
    function provisionLTEAccount($data) {
        $realm = substr($data['username'], strpos($data['username'], '@') + 1);
        $realm_data = $this->realm_model->get_realm_data_by_name($realm.".co.za");
        $connect_result = $this->is_connect_new_with_handler($realm_data);
        if (!$connect_result['result'])
            return $connect_result;

        return $this->provision_lte_account_new($connect_result['session'], $data);
    }

    //5.2.75  provisionLTEAccount
    function provision_lte_account_new($sess, $data){
        $data = array(
            'function' => 'provisionLTEAccount',
            'strSessionID' => $sess,
            'strUserName'  => $data["username"],
            'strPassword'     => $data["password"],
            'strEmailAddress'     => $data["email"],
            'intClassID'     => $data["class_id"],
            'strICCID'     => $data["sim"],
    ...
    function getClassesByRealm($realm) {
        ...
        return $this->get_classes_by_realm($connect_result['session'], $realm);
    }

    //5.2.75  provisionLTEAccount
    function get_classes_by_realm($sess, $realm){
        $data = array(
            'function' => 'classes',
            'sess' => $sess,
            'realm' => $realm
    ...
    //3.23 SIM SWAP
    function ownSimSwap($data) {
        $realm_data = $this->realm_model->get_realm_data_by_name("rest");

        $postData['data'] = json_encode($data);
        $postData['function'] = 'ownSimSwap';
        $postData['realm_data'] = json_encode($realm_data);

        $response = $this->curl_send($postData);

        return json_decode($response, true);
    }

    //3.20 accountMap
    function accountMap($username) {
        $realm_data = $this->realm_model->get_realm_data_by_name("rest");

        $postData['username'] = $username;
        $postData['function'] = 'accountMap';
        $postData['realm_data'] = json_encode($realm_data);
